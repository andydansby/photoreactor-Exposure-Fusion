...
public:
	...
	virtual void Process_Data2 (BYTE* pBGRA_out, BYTE* pBGRA_in1, BYTE* pBGRA_in2, int nWidth, int nHeight, UIParameters* pParameters)
	{
		...
		*/
		...
		//convert to the sRGB colorspace
		sRGBtoRGB(inputImage1,nWidth,nHeight);
		sRGBtoRGB(inputImage2,nWidth,nHeight);

		//convert to the HSL colorspace to measure saturation
		RGBtoHSL(inputImage1,nWidth,nHeight);
		RGBtoHSL(inputImage2,nWidth,nHeight);


		float* saturatedImage=new float[nWidth * nHeight * 4];
		initalizeArrayNeutralGrey(saturatedImage, nWidth, nHeight);
		...
		for (int x = 0; x < nWidth; x++)		
		{
			for (int y = 0; y < nHeight; y++)
			{
				...
				float hslH1 = inputImage1[nIdx + CHANNEL_R];
				float hslL1 = inputImage1[nIdx + CHANNEL_B];

				float hslH2 = inputImage2[nIdx + CHANNEL_R];
				float hslL2 = inputImage2[nIdx + CHANNEL_B];

				float hslH = (hslH1 + hslH2) / 2.0;
				float hslL = (hslL1 + hslL2) / 2.0;

				saturatedImage[nIdx + CHANNEL_R] = hslH;
				saturatedImage[nIdx + CHANNEL_B] = hslL;
			}
		}

		//compare which of the two images has the most saturation, pixel by pixel
		//the saturation layer is found in the GREEN channel
		for (int x = 0; x < nWidth; x++)		
		{
			for (int y = 0; y < nHeight; y++)
			{
				...
				float saturationImage1 = inputImage1[nIdx + CHANNEL_G];
				float saturationImage2 = inputImage2[nIdx + CHANNEL_G];

				float mostSaturated = max(saturationImage1,saturationImage2);

				saturatedImage[nIdx + CHANNEL_G] = mostSaturated;
			}
		}


		/*
		int* compairsonArray=new int[nWidth * nHeight * 4];
		//compare which of the two images has the most contrast, pixel by pixel
		for (int x = 0; x < nWidth; x++)		
		{
			for (int y = 0; y < nHeight; y++)
			{
		...
		delete [] compairsonArray;
		*/



		HSLtoRGB(saturatedImage,nWidth,nHeight);
		RGBtosRGB(saturatedImage,nWidth,nHeight);
		...
		delete [] saturatedImage;



			
		HSLtoRGB(inputImage1,nWidth,nHeight);
		HSLtoRGB(inputImage2,nWidth,nHeight);

		RGBtosRGB(inputImage1,nWidth,nHeight);
		RGBtosRGB(inputImage2,nWidth,nHeight);
