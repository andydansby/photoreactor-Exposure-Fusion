...
#define PARAM_FUSIONTYPE	3
#define PARAM_LUMA          4

#define PARAM_SWITCH	    5
#define PARAM_CHANNEL	    6


#define NUMBER_OF_USER_PARAMS 7
...
public:
	...
	//	AddParameter( PARAM_THETA , "Theta", .5, -1.0, 1.0, TYPE_SLIDER, 0.0);
		AddParameter( PARAM_FUSIONTYPE ,"Brightest CSE|???|???", 0, 0, 2, TYPE_ONEOFMANY, 0);
		AddParameter( PARAM_LUMA ,"RGB Average|Rec709|Exposure", 0, 0, 2, TYPE_ONEOFMANY, 0);
	...
	virtual void Process_Data2 (BYTE* pBGRA_out, BYTE* pBGRA_in1, BYTE* pBGRA_in2, int nWidth, int nHeight, UIParameters* pParameters)
	{
		...
		*/
		...
		int fusionType = GetValue(PARAM_FUSIONTYPE);
		int lumaType = GetValue(PARAM_LUMA);
			...
			for (int x = 0; x < nWidth; x++)
			{
				...
				{				
					...
					*/
					...
					//   ***IMPORTANT***
					//   *** BUILD AN ALPHA MASK WITH THE RESULTS, output to Blue channel
					...
					if (fusionType == 1)
					{
						//fusionImage1 [nIdx + CHANNEL_B] = (red11 * 0.212656) * (green11 * 0.715158) * (blue11 * 0.072186);
						//fusionImage2 [nIdx + CHANNEL_B] = (red22 * 0.212656) * (green22 * 0.715158) * (blue22 * 0.072186);
					}
					if (fusionType == 2)
					{
						//fusionImage1 [nIdx + CHANNEL_B] = (red11 * 0.299) * (green11 * 0.587) * (blue11 * 0.114);
						//fusionImage2 [nIdx + CHANNEL_B] = (red22 * 0.299) * (green22 * 0.587) * (blue22 * 0.114);
					}
...
// Alpha Mask test				
		...
		// meaning Contrast Saturation Exposure
		if (fusionType == 0)// does not use strength dials, averages of input
		{
			for (int x = 0; x < nWidth; x++)		
			{
				for (int y = 0; y < nHeight; y++)
		...
		if (fusionType == 1)// 
		{
			for (int x = 0; x < nWidth; x++)		
			{
				for (int y = 0; y < nHeight; y++)
				{
					...
					float redContrast = (red1 * contrast1) + (red2 * contrast2);
					float redSaturation = (red1 * saturation1) + (red2 * saturation2);
					float redExposure = (red1 * exposure1) + (red2 * exposure2);

					float greenContrast = (green1 * contrast1) + (green2 * contrast2);
					float greenSaturation = (green1 * saturation1) + (green2 * saturation2);
					float greenExposure = (green1 * exposure1) + (green2 * exposure2);

					float blueContrast = (blue1 * contrast1) + (blue2 * contrast2);
					float blueSaturation = (blue1 * saturation1) + (blue2 * saturation2);
					float blueExposure = (blue1 * exposure1) + (blue2 * exposure2);

					//cse array represents strength a pixel should have
					//outputRed = redContrast + redSaturation + redExposure;
					//outputGreen = greenContrast + greenSaturation + greenExposure;
					//outputBlue = blueContrast + blueSaturation + blueExposure;
					//outputRed = redContrast;
					//outputGreen = greenContrast;
					//outputBlue = blueContrast;
					//outputRed = redSaturation;
					//outputGreen = greenSaturation;
					//outputBlue = blueSaturation;
					outputRed = redExposure;
					outputGreen = greenExposure;
					outputBlue = blueExposure;
